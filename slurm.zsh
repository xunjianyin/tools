...
INTERACTIVE_SLURM_ARG="--nodes=1 --ntasks-per-node=1 --mem=30G -t 3:00:00 --pty zsh -i"

function islurm() {
    # Default values
    local partition="compsci-gpu"
    local gpu_model=""
    ...
    if [ -z "$gpu_model" ]; then
        ...
        local gpu_arg="--gres=gpu:1"
    else
        local gpu_arg="--gres=gpu:$gpu_model:1"
    fi
